...
const require_webAuthNError = require('../../errors/webAuthNError.js');
require('../../error.js');
...
async function webAuthnCreateCredential(publicKeyOptions) {
	try {
		...
		if (!credential) return {
			error: new require_webAuthNError.ClerkWebAuthnError("Browser failed to create credential", { code: "passkey_registration_failed" }),
...
async function webAuthnGetCredential({ publicKeyOptions, conditionalUI }) {
	try {
		...
		if (!credential) return {
			error: new require_webAuthNError.ClerkWebAuthnError("Browser failed to get credential", { code: "passkey_retrieval_failed" }),
			publicKeyCredential: null
		};
		return {
			publicKeyCredential: credential,
...
function handlePublicKeyError(error) {
	if (error.name === "AbortError") return new require_webAuthNError.ClerkWebAuthnError(error.message, { code: "passkey_operation_aborted" });
	if (error.name === "SecurityError") return new require_webAuthNError.ClerkWebAuthnError(error.message, {
...
function handlePublicKeyCreateError(error) {
	if (error.name === "InvalidStateError") return new require_webAuthNError.ClerkWebAuthnError(error.message, { code: "passkey_already_exists" });
	if (error.name === "NotAllowedError") return new require_webAuthNError.ClerkWebAuthnError(error.message, { code: "passkey_registration_cancelled" });
...
function handlePublicKeyGetError(error) {
	if (error.name === "NotAllowedError") return new require_webAuthNError.ClerkWebAuthnError(error.message, { code: "passkey_retrieval_cancelled" });
	return handlePublicKeyError(error);
}
